...
const avgEfficiencyFactor = (run?: Run) => {
    const eff =
        run?.split_stats.map((s) => s.efficiency).filter((x): x is number => x !== null) ?? [];
            ...
            <NotableRunCard :run="session.notableRuns?.runs.HOTTEST">
                <h5>
                    <span style="color: goldenrod"
                        ><i class="bi bi-thermometer-sun"></i> Hottest:
                    </span>
            ...
            <NotableRunCard :run="session.notableRuns?.runs.COLDEST">
                <h5>
                    <span style="color: steelblue"
                        ><i class="bi bi-thermometer-snow"></i> Coldest:
                    </span>
            ...
            <NotableRunCard :run="session.notableRuns?.runs.WETTEST">
                <h5>
                    <span style="color: #3366ff"
                        ><i class="bi bi-cloud-rain-heavy"></i> Wettest:
                    </span>
                    {{ session.notableRuns?.runs.WETTEST?.weather?.rain_mm }} mm/hour rainfall
                </h5>
            </NotableRunCard>
        </div>
        <div class="row gy-3 mt-3">
            <NotableRunCard :run="session.notableRuns?.runs.EARLIEST">
                <h5>
                    <span style="color: orange"><i class="bi bi-sunrise"></i> Earliest: </span>
                    {{
                        session.notableRuns?.runs.EARLIEST.start_time.toLocaleString(
                            currentLocale,
                            {
                                hour: "numeric",
                                minute: "numeric",
            ...
            <NotableRunCard :run="session.notableRuns?.runs.LATEST">
                <h5>
                    <span style="color: lightslategray"
                        ><i class="bi bi-moon-stars"></i> Latest:
                    </span>
                    {{
                        session.notableRuns?.runs.LATEST.start_time.toLocaleString(currentLocale, {
                            hour: "numeric",
                            minute: "numeric",
                        })
                    }}
        ...
        <div class="row gy-3 mt-3">
            <NotableRunCard :run="session.notableRuns?.runs.FASTEST">
                <h5>
                    ...
                    {{ durationFormatter(session.notableRuns!.runs.FASTEST!.average_pace) }} min/{{
                        distAbbr
                    }}
                </h5>
            </NotableRunCard>
            ...
            <NotableRunCard :run="session.notableRuns?.runs.MOST_EFFICIENT">
                <h5>
                    ...
                    {{
                        formatDec(avgEfficiencyFactor(session.notableRuns?.runs.MOST_EFFICIENT), 2)
                    }}
                    meters/heartbeat
                </h5>
            </NotableRunCard>
